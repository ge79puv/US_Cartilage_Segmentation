...
import math
from models.Attention import SEAttention, CBAM
...
class deeplabv3plus(nn.Module):
	def __init__(self, input_channel=None, num_classes=None):
		...
		self.cls_conv = nn.Conv2d(256, 2, 1, 1, padding=0)   
		...
		self.backbone = xception.Xception(os=16, input_channel=input_channel)
...
class deeplabv3plus_enhanced(nn.Module):
	def __init__(self, input_channel=None, num_classes=None):
		super(deeplabv3plus_enhanced, self).__init__()
		self.MODEL_NUM_CLASSES = num_classes
		self.backbone = None
		self.backbone_layers = None
		self.aspp = ASPP(dim_in=2048, dim_out=256, rate=16//16, bn_mom = 0.99)
		...
		self.attention1 = SEAttention(channel=512)
		self.attention2 = CBAM(channel=2304)			
		...
		self.cat_conv = nn.Sequential(
				nn.Conv2d(2304+48, 256, 3, 1, padding=1, bias=True),		
				nn.BatchNorm2d(256),
				nn.ReLU(inplace=True),
				nn.Dropout(0.5),
				nn.Conv2d(256, 256, 3, 1, padding=1, bias=True),
		...
		self.cls_conv = nn.Conv2d(256, 2, 1, 1, padding=0)   # self.MODEL_NUM_CLASSES=2
	...
	def forward(self, x, cla_cam, cls_features):

		cla_cam = resize(cla_cam.data.cpu().numpy(), (cla_cam.shape[0], cla_cam.shape[1], x.shape[2], x.shape[3]))
		cla_cam = torch.from_numpy(cla_cam).cuda()
		feature_cat0 = torch.cat([x, cla_cam], 1)

		x_bottom = self.backbone(feature_cat0)
		layers = self.backbone.get_layers()
		feature_aspp = self.aspp(layers[-1])		# Xception.layers[-1]的尺寸和cla_cam的MaskCN的Xception.layers[0]一致
		feature_aspp = self.dropout1(feature_aspp)
		
		feature_cat1 = torch.cat([feature_aspp, cls_features], 1)	
		# torch.Size([2, 256, 8, 10]) torch.Size([2, 2048, 8, 10])

		feature_cam = self.attention2(feature_cat1)					# torch.Size([2, 2304, 8, 10])
		feature_cam = self.upsample_sub(feature_cam)

		feature_shallow = self.shortcut_conv(layers[0])				# 48 channels
		feature_cat1 = torch.cat([feature_cam, feature_shallow], 1)
		result = self.cat_conv(feature_cat1)						# 2352 channels
		result = self.cls_conv(result)
		result = self.upsample4(result)
		return result
...
class Xception(nn.Module):
	...
	def forward(self, x):
		...
		x += res
		x = self.SepConv3(x)
		x = self.SepConv4(x)			# torch.Size([1, 2048, 15, 20])  cls_features
		self.layers.append(x)

		x = self.avgpool(x)				# torch.Size([1, 2048, 1, 1])
		x = x.view(x.size(0), -1)		# torch.Size([1, 2048])
		x = self.cls(x)					# torch.Size([1, 4])
		self.layers.append(x)
