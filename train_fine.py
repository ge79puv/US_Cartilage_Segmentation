...
def main():
    ...
    model_urls = {'CoarseSN': './results/CoarseSN/CoarseSN.pth', 'MaskCN': './results/EnhanceSN/MaskCN_updated.pth'}



    ############# Create mask-guided classification network.
    MaskCN = Xception(num_classes=NUM_CLASSES_CLS, input_channel=INPUT_CHANNEL)
    MaskCN.cuda()               # Don't need to configure optimizer for MaskCN
    ...
    EnhanceSN = deeplabv3plus_enhanced(num_classes=NUM_CLASSES_SEG, input_channel = 4)
    ...
    scheduler_lr = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'max', 
                                patience=5, verbose=1, factor=0.5, min_lr=1e-6)
    ...
    data_train_root_coursemask = './results/CoarseSN/train_old/'          
    data_train_list = './dataset/patient1_img_precise/cls/Training_cls_new.txt'
    trainloader = data.DataLoader(MyDataSet_seg(data_train_img_root, data_train_label_root, data_train_list, data_train_root_coursemask, crop_size=(w, h)),
    ...
    data_val_root_coursemask = './results/CoarseSN/validation_old/'          
    data_val_list = './dataset/patient1_img_precise/cls/Validation_cls_new.txt'
    valloader = data.DataLoader(MyDataSet_seg(data_val_img_root, data_val_label_root, data_val_list, data_val_root_coursemask, crop_size=(w, h)), batch_size=1, shuffle=False,
    ...
    data_test_root_coursemask = './results/CoarseSN/test_old/'          
    data_test_list = './dataset/patient1_img_precise/cls/Testing_cls_new.txt'
    testloader = data.DataLoader(MyDataSet_seg(data_test_img_root, data_test_label_root, data_test_list, data_test_root_coursemask, crop_size=(w, h)), batch_size=1, shuffle=False,
                                num_workers=2, pin_memory=True, drop_last=True)
    ...
    for epoch in range(EPOCH):
        ...
        for i_iter, batch in tqdm(enumerate(trainloader)):
            ...
            EnhanceSN.train()
            preds = EnhanceSN(images, cla_cam, cls_features)

            term1 = Loss_func(preds, labels[:, 0:2, :, :])
            term = term1
            ...
            train_loss_bce.append(term1.cpu().data.numpy()) 
            # train_loss_haus.append(term2.cpu().data.numpy()) 


        print("train_epoch%d: lossTotal=%f, Jaccard=%f, loss_bce=%f, loss_haus=%f \n" % (epoch, np.nanmean(train_loss_total), np.nanmean(train_jac)
                                                        , np.nanmean(train_loss_bce), np.nanmean(train_loss_haus)))

        ############# Start the validation

        val_cams = val_mode_cam(valloader, MaskCN)

        [vacc, vdice, vsen, vspe, vjac_score, vhd_loss, vtotal_loss] = val_mode_seg(valloader, val_cams, EnhanceSN, MaskCN, path, epoch)
        ...
        if total_score > best_score:
            ...
            best_model = EnhanceSN.state_dict()
            print('Best model score : %.4f'%(best_score))
            torch.save(best_model, path + 'EnhanceSN' + '.pth')
            torch.save(MaskCN.state_dict(), path + 'MaskCN_updated' + '.pth')


    Loss = np.array(val_loss)
    np.save('./results/EnhanceSN/Loss_DICE', Loss)

    HD_Loss = np.array(val_hd_loss)
    np.save('./results/EnhanceSN/HD_Loss_Dice', HD_Loss)

    IOU = np.array(val_jac)
    np.save('./results/EnhanceSN/IOU_Dice', IOU)  

    Dice = np.array(val_dice)
    np.save('./results/EnhanceSN/Dice_Dice', Dice)      
    ...
    plt.title('IOU')
    plt.legend()
    plt.savefig(path + 'IOU_Dice')
    plt.show()

    
    ############# Start the test
    pretrained_dict = torch.load(r'./results/EnhanceSN/EnhanceSN.pth')    
    EnhanceSN.load_state_dict(pretrained_dict) 

    pretrained_dict = torch.load(r'./results/EnhanceSN/MaskCN_updated.pth')     
    MaskCN.load_state_dict(pretrained_dict) 

    test_cams = val_mode_cam(testloader, MaskCN)      
    epoch = 666
    [tacc, tdice, tsen, tspe, tjac_score, thd_loss, ttotal_loss] = val_mode_seg(testloader, test_cams, EnhanceSN, MaskCN, path, epoch)
