...
def main():
    ...
    model_urls = {'CoarseSN': './results/CoarseSN/CoarseSN_old.pth', 'MaskCN': './results/MaskCN/Cls_old_e49.pth'}
    ...
    ############# Create enhanced segmentation network for cartilage segmentation.
    EnhanceSN_soft = deeplabv3plus_enhanced(num_classes=NUM_CLASSES_SEG, input_channel = 4)
    optimizer = torch.optim.Adam(EnhanceSN_soft.parameters(), lr=LEARNING_RATE)
    EnhanceSN_soft.cuda()
    ...
    net_dict = EnhanceSN_soft.state_dict()
    pretrained_dict = {k: v for k, v in pretrained_dict.items() if (k in net_dict) and (v.shape == net_dict[k].shape)}
    
    net_dict.update(pretrained_dict)
    EnhanceSN_soft.load_state_dict(net_dict)

    EnhanceSN_soft.train()
    ...
    ################## Load training and validation data of cartilage
    ...
    trainloader_soft = data.DataLoader(MyDataSet_seg(data_train_img_root, data_train_label_root, data_train_list, data_train_root_coursemask, crop_size=(w, h)),
    ...
    valloader_soft = data.DataLoader(MyDataSet_seg(data_val_img_root, data_val_label_root, data_val_list, data_val_root_coursemask, crop_size=(w, h)), batch_size=1, shuffle=False,
    ...
    testloader_soft = data.DataLoader(MyDataSet_seg(data_test_img_root, data_test_label_root, data_test_list, data_test_root_coursemask, crop_size=(w, h)), batch_size=1, shuffle=False,
                                num_workers=2, pin_memory=True, drop_last=True)
    ...
    if not os.path.isdir(path):
        os.mkdir(path)
    f_path = path + 'output_soft_alpha.txt'
    ...
    for epoch in range(EPOCH):
        ...
        for i_iter, batch in tqdm(enumerate(trainloader_soft)):

            images, coarsemask, labels, name = batch    # torch.Size([2, 3, 240, 320])
            images = images.cuda()
            coarsemask = coarsemask.cuda()              
            labels = labels.cuda()                      
            ...
            EnhanceSN_soft.train()
            preds = EnhanceSN_soft(images, cla_cam, cls_features)
        ...
        val_cams = val_mode_cam(valloader_soft, MaskCN)

        [vacc, vdice, vsen, vspe, vjac_score, vhd_loss, vtotal_loss] = val_mode_seg(valloader_soft, val_cams, EnhanceSN_soft, MaskCN, path, epoch)
        ...
        plt.savefig(os.path.join(path, 'jaccard_soft_alpha.png'))
        plt.clf()
        plt.close()
        plt.show()
        ...
        if total_score > best_score:
            ...
            best_model = EnhanceSN_soft.state_dict()
            print('Best model score : %.4f'%(best_score))
            torch.save(best_model, path + 'EnhanceSN_soft_alpha' + '.pth')
            torch.save(MaskCN.state_dict(), path + 'MaskCN_soft_alpha_updated' + '.pth')



    Loss = np.array(val_loss)
    np.save('./results/EnhanceSN/Loss_soft_alpha', Loss)

    HD_Loss = np.array(val_hd_loss)
    np.save('./results/EnhanceSN/HD_Loss_soft_alpha', HD_Loss)

    IOU = np.array(val_jac)
    np.save('./results/EnhanceSN/IOU_soft_alpha', IOU)  

    Dice = np.array(val_dice)
    np.save('./results/EnhanceSN/Dice_soft_alpha', Dice)      
    ...
    plt.legend()
    # plt.axis([0, None, 0, 1])
    plt.savefig(path + 'Loss_soft_alpha')
    plt.show()
    ...
    plt.title('IOU')
    plt.legend()
    plt.savefig(path + 'IOU_soft_alpha')
    plt.show()

    
    
    ############# Start the test
    pretrained_dict = torch.load(r'./results/EnhanceSN/EnhanceSN_soft_alpha.pth')    
    EnhanceSN_soft.load_state_dict(pretrained_dict) 

    pretrained_dict = torch.load(r'./results/EnhanceSN/MaskCN_soft_alpha_updated.pth')     
    MaskCN.load_state_dict(pretrained_dict) 

    test_cams = val_mode_cam(testloader_soft, MaskCN)      
    [tacc, tdice, tsen, tspe, tjac_score, thd_loss, ttotal_loss] = val_mode_seg(testloader_soft, test_cams, EnhanceSN_soft, MaskCN, path, epoch)
